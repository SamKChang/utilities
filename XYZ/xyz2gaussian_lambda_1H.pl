...
my $lambda = $ARGV[2];
my $lambdaN= sprintf("%03d",$lambda*100);
...
%nproc=1
%chk=$pwd/$stem
# rohf gen 6d 10f nosymm Scf(maxcycle=1000,verytight) int(grid=ultrafine) charge IOp(2/12=3) pop=full
...
#{
  $a=0;
#  printf("%s-Bq", $ATOMS2[$a][0]);
#  for(my $i=1;$i<4;$i++){printf(" % 12.8f",$ATOMS2[$a][$i]);}
#  print "\n";
  print "\n";
  for(my $i=1;$i<4;$i++){printf("% 12.8f",$ATOMS[$a][$i]);}
  printf("  % 5.3f\n",-atom_ve($ATOMS[$a][0]) * $lambda);
  for(my $i=1;$i<4;$i++){printf("% 12.8f",$ATOMS2[$a][$i]);}
  printf("  % 5.3f\n",atom_ve($ATOMS2[$a][0])*$lambda);
#}
