...
static int *AOMAP, *AGMAP, *MAP, *lmMAP, *rCrg, *tCrg;
static double *PEXP, *CTRC, *EVa, *EWa, *Gmo, *rCrd, *tCrd;
...
*******************************************************/
  FILE *fCMNBasis;
  FILE *fAOMAP, *fAGMAP, *fPEXP, *frCrd, *fCTRC, *fMAP, *flmMAP;
  ...
  fAOMAP = fopen("AOMAP.pltmp","r");
  fAGMAP = fopen("AGMAP.pltmp","r");
  flmMAP = fopen("lmMAP.pltmp","r");
  fPEXP = fopen("PEXP.pltmp","r");
  frCrd = fopen("COORD.pltmp","r");
  fCTRC = fopen("CTRC.pltmp","r");
  fMAP = fopen("MAP.pltmp","r");
  ...
  AOMAP = (int *) malloc(Nbasis * sizeof(int));
  AGMAP = (int *) malloc((Ngaussian+1) * sizeof(int));
  MAP = (int *) malloc(Ngaussian * sizeof(int));
  lmMAP = (int *) malloc(3 * Nbasis * sizeof(int));

  PEXP = (double *) malloc(Ngaussian * sizeof(double));
  CTRC = (double *) malloc(Ngaussian * sizeof(double));
  ...
  for(i=0;i<Nbasis;i++){
    fscanf(fAOMAP,"%d",&AOMAP[i]);

    fscanf(flmMAP,"%d",&lmMAP[3*i]);
    fscanf(flmMAP,"%d",&lmMAP[3*i+1]);
    fscanf(flmMAP,"%d",&lmMAP[3*i+2]);
  ...
  for(i=0;i<Ngaussian;i++){
    fscanf(fAGMAP,"%d",&AGMAP[i]);
    fscanf(fMAP,"%d",&MAP[i]);
    fscanf(fPEXP,"%lf",&PEXP[i]);
    fscanf(fCTRC,"%lf",&CTRC[i]);
  }
  fscanf(fAGMAP,"%d",&AGMAP[Ngaussian]);
