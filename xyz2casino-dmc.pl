...
# DMC parameters
my $WALKER = 4096;
my $strDst = sprintf("%-14d",$ARGV[2]); 
my $strDch = sprintf("%-14d",ceil($ARGV[2]/5000));
my $strDwk = sprintf("%-14d",$WALKER);
my $strDti = sprintf("%-14d",ceil($WALKER*2.5));
my $strDdt = sprintf("%-14d",$ARGV[3]);
...
sp_blips          : T              #*! Single precision blip functions
...
dmc_stats_nstep   : $strDst #*! Number of steps (Integer)
dmc_stats_nblock  : $strDch #*! Number of checkpoints (Integer)
dmc_target_weight : $strDwk #*! Total target weight in DMC (Real)
dmc_trip_weight   : $strDti #*! Alert weight for catastrophe
dtdmc             : $strDdt #*! DMC time step (Real)
