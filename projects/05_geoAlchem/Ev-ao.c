...
static double *EVa, *EWa, *Vao, *Vmo, *rCrd;
...
*******************************************************/
  ...
  Vao = (double *) malloc(Nbasis*Nbasis * sizeof(double));
  Vmo = (double *) malloc(Nbasis*Nbasis * sizeof(double));
...
/***************************************
*  construct nuclear repulsion energy  *
***************************************/
  for(i=0;i<Natom;i++){
    for(j=i+1;j<Natom;j++){
      double Rij = 0;
...
/**********************************************
*  construct matrix element of V in MO basis  *
**********************************************/
  int ami, amj, ama, amb;
  for(ami=0;ami<Nbasis;ami++){
    for(amj=0;amj<Nbasis;amj++){
      int mo_ij = Nbasis*ami + amj;
      Vmo[mo_ij] = 0;
      for(ama=0;ama<Nbasis;ama++){
        for(amb=0;amb<Nbasis;amb++){
          int ao_ab = Nbasis*ama + amb;
          int ia = ami*Nbasis + ama;
          int jb = amj*Nbasis + amb;
          Vmo[mo_ij] += EVa[ia]*Vao[ao_ab]*EVa[jb];
        }
      }
    }
  }
...
//          double V = Vmo[ia]*Vao[ib]*Vao[ab]*Vao[bc];
...
//          double V = Vmo[ia]*Vao[ja]*Vao[ik]*Vao[jk];
...
//          double V = Vmo[ia]*Vao[jb]*Vao[ij]*Vao[ab];
//          double E = Cia*Cjb*Cib;
//          d4E -= 2*V/E;
//
//          double V1 = Vmo[ia]*Vao[jb]*Vao[ib]*Vao[ia];
