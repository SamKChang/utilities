...
for(my $dummy=scalar(@ARGV);$dummy>=0;$dummy--){
  my ($sysCrg,$sysID);
  if($dummy<scalar(@ARGV)){
    print '$new_job',"\n";
    $sysCrg = -($crg - $charge[$dummy]);
    $sysID = $ARGV[$dummy];
    $sysID =~ s/S_/r/;
    $sysID =~ s/\.xyz//;
  }else{
  ...
  #print $NAt,"\n\n";
  printHeader($sysID, $sysCrg);
  for(my $f=0;$f<scalar(@ARGV);$f++){
    if($dummy!=$f){
      for(my $a=0;$a<$NA[$f];$a++){
        printf("%-3s ",atom_a2n($XYZ[$f][$a][0]));
        for(my $i=1;$i<=3;$i++){
          printf("% 12.8f ",$XYZ[$f][$a][$i]);
        }
        print "\n";
      }
    }else{
...
sub printHeader{
print <<EOF;
! HF DLPNO-CCSD(T)  aug-cc-pVTZ aug-cc-pVTZ/C XYZFile PModel
%id "$_[0]"
%base "$_[0]"
%scf
  Convergence
